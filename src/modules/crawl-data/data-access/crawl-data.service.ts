...
export class CrawlDataService {
	...
	async getTopHeadlines(query: getTopHeadlinesReqDto) {
		...
		return this.httpService
			.get<ICrawlDataRes>(`${this.configService.get<string>(EEnvKey.CRAWL)}top-headlines`, {
				params: { ...query, apiKey },
			})
			.pipe(
				map(response => response.data),
				catchError((error: { response: { data: IErrorData } }) => {
	...
	async getSearchNews(query: getSearchNewsReqDto) {
		return this.httpService
			.get<ICrawlDataRes>(`${this.configService.get<string>(EEnvKey.CRAWL)}everything`, { data: { ...query, apiKey } })
			.pipe(
				map(response => response.data),
				catchError((error: { response: { data: IErrorData } }) => {
					const { data } = error.response;
	...
	async getNewsSources(query: getNewsSourcesReqDto) {
		return this.httpService
			.get<ICrawlDataRes>(`${this.configService.get<string>(EEnvKey.CRAWL)}top-headlines/sources`, {
				data: { ...query, apiKey },
			})
			.pipe(
				map(response => response.data),
